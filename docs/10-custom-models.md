...
## On-device компиляция (DLC → context binary на телефоне)

Device → **On-device compile** компилирует установленные DLC-модели под твой
конкретный чип прямо на телефоне и сохраняет как быстрые context binary в
`models/custom/` — там их подхватывает Benchmark/Vision.

Как это работает: DLC и так компилируется (compose + finalize) под арку при
каждой загрузке — это и есть медленная часть. Кнопка делает это один раз и
сериализует готовый контекст через `QnnContext_getBinary` в `.bin`. Дальше
этот `.bin` грузится напрямую (`contextCreateFromBinary`), без prepare.

- `ONNX → DLC` по-прежнему только на ПК (`qairt-converter` = x86), см. выше.
- `DLC → context binary` — это уже на устройстве (нативный `SerializeContext`
  в JNI; `qnn-context-binary-generator` тоже есть в `bin/aarch64-android`).
- Project Panama (`java.lang.foreign`) на Android ART недоступен — нативка
  только через JNI/NDK, что у нас и используется.
